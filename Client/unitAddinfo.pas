...
{           arrival:=frmManeger.ZamenaDotNakoma(arrival);
           charge:=frmManeger.ZamenaDotNakoma(charge);
           cost:=frmManeger.ZamenaDotNakoma(cost);}

           qry.ParamByName('edate').AsString:=Edate;
           qry.ParamByName('Enum_doc').AsString:=Enum_doc;
           qry.ParamByName('EFrom_Whom').AsString:=EFrom_Whom;
           qry.ParamByName('cost').AsFloat:=StrTofloat(cost);
           qry.ParamByName('Arrival').AsFloat:=StrTofloat(Arrival);
           qry.ParamByName('Charge').AsFloat:=StrTofloat(Charge);
           qry.ParamByName('elem_id').AsInteger:=elem_id;
           qry.ParamByName('user').AsInteger:=elem_id;
           qry.ExecSQL;
           qry.SQL.Text:=tmpStr;
           result:=true;
      finally
...
begin
     if (_Elem_name ='') then
       begin
          messageDlg('Необходимо выбрать "Элемент"...',mtInformation,[MbOk],0);
          exit;
         end;
    with dm do
    begin
     try
        try
         ...
         if  add_info(_elem_id,_elem_name,earrival.Text,echarge.Text,ecost_for_one.Text,efrom_whom.Text,edate.Text,enum_doc.Text,main._login,qryaddInfo) then
...
begin
   try
      try
        if   _elem_name = '' then showmessage('Выбирите "Элемент"') else
         with dm do
          begin
...
begin
 ...
 frmCard.elem_name:=_elem_name;
  ...
  dm.qryInfo_by_elem.ParamByName('element_id').AsInteger:=_elem_id;
