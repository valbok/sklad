...
begin
  try
     try
       ...
       qry.ParamByName('link').AsString:=link;
       qry.ParamByName('linkInfo').AsString:=linkinfo;
...
with dm do
  ...
  try
     try
       ...
       if  add_link(_Elem_id,elink.text,memLinkinfo.text,main._login,qryAddlink) then
