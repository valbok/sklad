...
if gridZakazi.DataSource=dsqryZakazi_by_user then
  begin
   if messageDlg('Вы уверены, что желаете удалить Заказ?'+#13#10+
        ...
        'От кого: "'+QryZakazi_by_user.fieldByName('from_whom').asstring+'"'+#13#10+
        'Для какой цели: "'+QryZakazi_by_user.fieldByName('for_what').asString+'"'+#13#10+
        'Дата: "'+dateToStr(QryZakazi_by_user.fieldByName('thedate').asdateTime)+'"'+#13#10+
  ...
  if gridZakazi.DataSource=dsQryRequests_by_zakaz then
  begin
   if messageDlg('Вы уверены, что желаете удалить Требование?'+#13#10+
        '№: "'+intToStr(QryRequests_by_zakaz.fieldByName('request_id').asinteger)+'"'+#13#10+
        'Наименование, сорт, размер: "'+QryRequests_by_zakaz.fieldByName('name').asstring+'"'+#13#10+
        'Количество: "'+floatToStr(QryRequests_by_zakaz.fieldByName('col').asfloat)+'"',
