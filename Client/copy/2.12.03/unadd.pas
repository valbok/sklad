...
type
  TfrmManeger = class(TForm)
    ...
    procedure miDaliClick(Sender: TObject);
...
//   anim.Active:=true;
  ...
  begin
    if frmFindToManeger.existValue then
     if not miDali.Checked then
      LocateComponent(frmFindToManeger.value) else
        LocateNextComponent(frmFindToManeger.value)
  end else
  if grid.DataSource=dsQryType_by_comp then
  begin
    if frmFindToManeger.existValue then
      if not miDali.Checked then
      LocateType(frmFindToManeger.value) else
        LocateNextType(frmFindToManeger.value)
  end;
  if grid.DataSource=dsQryElem_by_type then
  begin
     if frmFindToManeger.existValue then
      if not miDali.Checked then
       LocateElem(frmFindToManeger.value) else
         LocateNextElem(frmFindToManeger.value)
  end;
  finally
     Grid.Cursor:=crDefault;
//       Anim.Active:=false;
...
procedure TfrmManeger.miDaliClick(Sender: TObject);
begin
 if miDali.Checked then
   miDali.Checked:=false else
    miDali.Checked:=true;
end;
