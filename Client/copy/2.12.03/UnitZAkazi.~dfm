object frmZakazi: TfrmZakazi
  Left = 190
  Top = 119
  Width = 544
  Height = 375
  Caption = #1047#1072#1082#1072#1079#1099' - '#1090#1088#1077#1073#1086#1074#1072#1085#1080#1103
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  Menu = mmZakaz
  OldCreateOrder = False
  PixelsPerInch = 96
  TextHeight = 13
  object pnlMain: TPanel
    Left = 0
    Top = 0
    Width = 536
    Height = 328
    Align = alClient
    BevelInner = bvRaised
    BevelOuter = bvLowered
    BevelWidth = 2
    BorderWidth = 2
    BorderStyle = bsSingle
    TabOrder = 0
    object pnlChildMain: TPanel
      Left = 6
      Top = 38
      Width = 520
      Height = 280
      Align = alClient
      BevelInner = bvLowered
      BevelOuter = bvLowered
      BorderWidth = 1
      TabOrder = 0
      object gridZakazi: TDBGrid
        Left = 3
        Top = 3
        Width = 514
        Height = 274
        Align = alClient
        DataSource = DM.dsCdsZAkazi
        Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
        PopupMenu = pmZakaz
        TabOrder = 0
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -11
        TitleFont.Name = 'MS Sans Serif'
        TitleFont.Style = []
        OnDblClick = gridZakaziDblClick
        OnKeyPress = gridZakaziKeyPress
        OnTitleClick = gridZakaziTitleClick
      end
    end
    object pnlTop: TPanel
      Left = 6
      Top = 6
      Width = 520
      Height = 32
      Align = alTop
      TabOrder = 1
      object sbBack: TSpeedButton
        Left = 5
        Top = 6
        Width = 65
        Height = 22
        Caption = #1053#1072#1079#1072#1076
        Flat = True
        Glyph.Data = {
          
          FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
        OnClick = sbBackClick
      end
      object Label1: TLabel
        Left = 88
        Top = 10
        Width = 46
        Height = 13
        Caption = #1058#1072#1073#1083#1080#1094#1072':'
      end
      object lblTable: TLabel
        Left = 136
        Top = 10
        Width = 39
        Height = 13
        Caption = #1047#1072#1082#1072#1079#1099
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clNavy
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
      end
      object pnlRight: TPanel
        Left = 432
        Top = 1
        Width = 87
        Height = 30
        Align = alRight
        BevelOuter = bvNone
        TabOrder = 0
        object btnClose: TBitBtn
          Left = 8
          Top = 3
          Width = 75
          Height = 25
          Caption = #1047#1072#1082#1088#1099#1090#1100
          TabOrder = 0
          OnClick = btnCloseClick
          Glyph.Data = {
            
            FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
        end
      end
      object Anim: TAnimate
        Left = 247
        Top = 2
        Width = 80
        Height = 29
        Active = False
        AutoSize = False
        FileName = 'I:\element\Client\reports\S.avi'
        StopFrame = 29
      end
    end
  end
  object pmZakaz: TPopupMenu
    OnPopup = pmZakazPopup
    Left = 328
    Top = 64
    object miDel: TMenuItem
      Caption = #1059#1076#1072#1083#1080#1090#1100
      Enabled = False
      OnClick = miDelClick
    end
    object miRefresh: TMenuItem
      Caption = #1054#1073#1085#1086#1074#1080#1090#1100
      OnClick = miRefreshClick
    end
    object N2: TMenuItem
      Caption = '-'
    end
    object miImportToWord: TMenuItem
      Caption = #1048#1084#1087#1086#1088#1090' '#1074'  MS Word'
      OnClick = miImportToWordClick
    end
  end
  object mmZakaz: TMainMenu
    Left = 120
    Top = 96
    object miShowZakazi: TMenuItem
      Bitmap.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      Caption = #1055#1086#1082#1072#1079#1072#1090#1100' '#1079#1072#1082#1072#1079#1099
      OnClick = miShowZakaziClick
    end
    object N1: TMenuItem
      Caption = '&'#1047#1072#1082#1072#1079#1099
      OnClick = N1Click
    end
  end
  object EkRTFZakaz: TEkRTF
    InFile = 'reports\template\zakaz.rtf'
    OutFile = 'outfile.doc'
    TrueValue = 'True'
    FalseValue = 'False'
    Charset = DEFAULT_CHARSET
    Lang = 0
    Options = [eoGraphicsBinary, eoDotAsColon]
    DisableControls = True
    Left = 216
    Top = 120
  end
end
