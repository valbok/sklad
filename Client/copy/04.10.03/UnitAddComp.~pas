...
begin
   ...
   if messageDlg('Вы уверены, что желаете удалить комплектующее?'+#13#10+'"'+comp_name+'"'+#13#10+'При этом удаляться все записи из других таблиц связаные с этим комплектующим!',mtConfirmation,[mbYes,MbNo],0) = mrYes then
          begin
            qry.SQL.text:=sqlText+'delete_component('''+comp_name+''')';
            qry.ExecSQL;
            showmessage('Комплектующее "'+Comp_name+'" удалено!');
            result:=true;
          end;
