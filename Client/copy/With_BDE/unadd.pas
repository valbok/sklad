...
{       except
          messageDlg('Error exec SQL! Возможно нет прав или такая запись уже существеут!',mtError,[MbOk],0);
       end;}
...
//               NavigatorTypes.BtnClick(nbrefresh);
                refresh;
{           except
               messageDlg('Error execute Query for types. Возможно нет прав',mtError,[MbOk],0);
               qryWork.SQL.Text:=tmpStr;
           end;}
           qryWork.SQL.Text:=tmpStr;
...
btnFindComponent.Enabled:=false;
 if (dm.IBDatabase.Connected=true) and (not dm.tblComponents.Locate('name',editComponentName.text,[loPartialKey]))
    then
      MessageDlg('Компонент "'+editComponentName.text+'" не найден!',mtInformation,[mbOk],0);
...
begin
    btnFindType.Enabled:=false;
     if (dm.IBDatabase.Connected) and (not dm.tblTypes.Locate('name',edtTypeName.text,[loPartialKey]))
       then
         MessageDlg('Тип с именем "'+edtTypeName.text+'" не найден!',mtInformation,[mbOk],0);
...
i:integer;
begin
  if cbFindField.Text = '' then showmessage('Выберите по какому полю производится поиск')
    else
    begin
...
{if ((Ecost_for_one.Text <> '') or (Ecost_for_one.Text <> '0'))
       and ((Echarge.Text <>'') or (Echarge.Text<>'0')) then
               begin
                  messageDlg('ельзя пустить в расход за деньги! Поле цена',mtError,[MbOk],0);
                  exit;
                end;}
       ...
       begin
         if (cbElem.Text ='') then
           begin
              messageDlg('Необходимо заполнить поле "Элемент"...',mtError,[MbOk],0);
              exit;
           end;
...
//               refresh;
  {         except
               messageDlg('Error run add Info by element. Возможно неправельный синтаксис!',mtError,[MbOk],0);
               qryWork.SQL.Text:=tmpStr;
 //             exit;
           end;}
           qryWork.SQL.Text:=tmpStr;
...
r:real;
begin
  if cbFindFieldInfo.Text = '' then showmessage('Выберите по какому полю производится поиск')
    else
    begin
...
//    showmessage(qryWork.SQL.Text);
                          qryWorkBDE.ExecSQl;
    qryWorkBDE.SQL.Text:='insert into Card (thedate,ostatok,name,type_name,from_whom)  values('''+dateToStr(qryConsole.FieldByName('date_of_srez').asDateTime)+''','+ostatok+','''+nam+''','''+type_name+''',''Остаток'')';
                          qryWorkBDE.ExecSQl;
