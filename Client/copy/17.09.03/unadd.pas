 ...
 try
    ...
    //    btnFindType.Enabled:=false;
     if (dm.IBDatabase.Connected) and (not dm.tblTypes.Locate('type_id',id,[loPartialKey]))
       then
         MessageDlg('Тип под номером "'+intToStr(id)+'" не найден!',mtInformation,[mbOk],0);
    finally
...
begin
  ...
  begin
    if grid.DataSource =dsComp then
    begin
{      miEdit.Caption:='Редактировать "'+grid.SelectedField.Text+'"';
      miDelete.Caption:='Удалить "'+grid.SelectedField.Text+'"';}
