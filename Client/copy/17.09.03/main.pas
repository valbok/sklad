...
type
  TfrmMain = class(TForm)
    ...
    procedure dbLinkDblClick(Sender: TObject);
...
implementation
uses dmo, DB, unadd, unitConsole, UnitFind, unitOstatki,unitAddRecords,
  unitPassword;
...
try
  ...
  StatusBar.Panels[0].Text:='Выполнение инструкции...';
  StatusBar.Refresh;
      try
          l:=start.level;
  ...
  begin
    ...
    start.Expand(false);
     ...
     begin    if l = 0 then
       ...
       StatusBar.Panels[0].Text:='Выполнено.';
       StatusBar.Refresh;
   end;
 except
  on E:Exception do
...
//                        showmessage(qryWork.SQL.text);
        ...
        typ:    begin
        //тут не правильно удаление <GetRecordName()>
...
begin
  frmManeger.btnShowAddForm.click;
end;

procedure TfrmMain.dbLinkDblClick(Sender: TObject);
var s:pchar;
begin
   if dbLink.Text <> '' then
   begin
...
begin
 try
   try
   treeStrDb.Cursor:=crHourGlass;
       StatusBar.Panels[0].Text:='Создание отчёта об остатках';
       StatusBar.Refresh;
  case levl of
