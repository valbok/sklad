...
type
  TfrmAddZakaz = class(TForm)
    ...
    function add_zakaz(from_whom,for_what:string;thedate:tdatetime;user:string;var qry,qrymax:TibQuery;var zakaz_id:integer):bool;
...
function TfrmAddzakaz.add_zakaz(from_whom,for_what:string;thedate:tdatetime;user:string;var qry,qrymax:TibQuery;var zakaz_id:integer):bool;
begin
   result:=false;
   if qry.Active then qry.Close;
   qry.ParamByName('from_whom').AsString:=from_whom;
   qry.ParamByName('for_what').AsString:=for_what;
   qry.ParamByName('thedate').AsDateTime:=thedate;
   qry.ParamByName('user').AsString:=user;
//   qry.SQL.Text:='execute procedure add_zakaz('''+from_whom+''','''+for_what+''','''+thedate+''','''+user+''')';
   qry.ExecSQL;
   qrymax.Open;
   zakaz_id:=qrymax.fieldByName('max').AsInteger;
   result:=true;
end;
...
try
  ...
  if  add_zakaz(edtFrom_whom.Text,edtFor_what.Text,StrToDate(edtTheDate.Text),main._login,dm.qryaddzakaz,dm.qrymaxZakaz_id,zakaz_id) then
